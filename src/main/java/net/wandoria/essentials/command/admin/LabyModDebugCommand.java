...
public class LabyModDebugCommand {
    ...
    public void listLabyModPlayers(Source source) {
        ...
        for (var player : labyPlayers) {
            ...
            var addons = player.installedAddons().getInstalledAddons();
            if (addons.isEmpty()) {
            ...
            for (var addon : addons) {
                ...
                source.source().sendMessage(Text.deserialize("     <gray>- <active> <hl><namespace></hl> <dark_gray>(Version: <hl><version></hl>)</dark_gray> <button>",
                        ...
                        Placeholder.unparsed("active", addon.isEnabled() ? "<green>●" : "<red>●"),
    ...
    public void disableLabyModAddon(Source source, Player player, String addonNamespace) {
        ...
        labyPlayer.disableAddons(addonNamespace);
        source.source().sendMessage(Text.deserialize("<prefix> <green>Das Addon <hl>" + addonNamespace + "</hl> wurde für Spieler <hl>" + player.getName() + "</hl> deaktiviert."));
    }
